...
# Images
from ImageUtilities import imageReadL, createImageF, createImageL, showImageL, plotImageL, createVectorF
...
def plotShapeinImage(shape, centre, width, height, figureSize = 5):
    segmentsImage = createImageL(width, height)
    numPoints = len(shape[0])
    for p in range(0, numPoints):
        y,x = int(centre[0]+shape[0,p]), int(centre[1]+shape[1,p])
        if x > 0 and y > 0 and x < width and y < height:
            segmentsImage[y,x] = 255
    plotImageL(segmentsImage, figureSize)
