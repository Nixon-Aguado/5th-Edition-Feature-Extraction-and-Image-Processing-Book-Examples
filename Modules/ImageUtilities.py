...
# Read an image returning an array with 3 float color components
def imageReadRGBF(fileName):   
    inputImage = Image.open(fileName)
    width, height = inputImage.size

    inputArray = array(inputImage)
    inputArray = inputArray[:, :, 0:3] / 255.0
...
# Show an array containing image data
def plotImageRGB(image,  figureSize = 5):
    # Create output and show
    outputImage = Image.fromarray(image, 'RGB')
    plt.rcParams["figure.figsize"] = (figureSize, figureSize)
    plt.axis("off")
    plt.rcParams.update()
...
# Show an array containing image data
def plotImageRGBF(image,  figureSize = 5):
    plt.rcParams["figure.figsize"] = (figureSize, figureSize)
    plt.axis("off")
    plt.rcParams.update()
    plt.imshow(image)
...
# Show an array containing image data
def plotImageL(image, figureSize = 5):
    # Create output and show
    outputImage = Image.fromarray(image, 'L')
    plt.rcParams["figure.figsize"] = (figureSize, figureSize)
    plt.axis("off")
...
# Show an array containing image data
def plotImageF(image, maxScale = 1, figureSize = 5):
    height = len(image)
    width = len(image[0])
