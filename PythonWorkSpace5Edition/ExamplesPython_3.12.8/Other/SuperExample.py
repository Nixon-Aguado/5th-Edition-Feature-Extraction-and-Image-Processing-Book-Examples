...
Chapter 7: Simple linear interactive clustering 
'''

# Image utility functions
from ImageSupport import imageReadL, createImageF, createImageRGB, imageReadRGB, \
                         showImageRGB, createImage2I, createImageUV, imageSaveRGB
from FeatureExtractionUtilities import createSobelKernel, applyKernelMA
# Iteration
...
gradient, _ = applyKernelMA(inputImageL, sobelX, sobelY, normalizeMagnitude)
...
for i in range(5,34,1):
    ...
    regionSide = int(sqrt(width *  height / (i*i))) 
    if regionSide % 2 == 0: regionSide -= 1
    halfRegionSide = (regionSide - 1.0) / 2.0
    regW, regH = 1 + width / regionSide, 1 + height / regionSide
    ...
    imageSaveRGB(resultRegions, "../../../Images/Chapter7/Results/SequenceSuper/fish" + str(j) + ".png")
